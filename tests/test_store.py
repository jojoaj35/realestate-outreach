...
def test_listing_to_row_status_by_score():
    low = listing_to_row({
        "listing_id": "1", "score": 0.3,
        "agent_name": "Jane Agent", "agent_phone": "(512) 555-0100",
    })
    high = listing_to_row({
        "listing_id": "2", "score": 0.9,
        "agent_name": "Bob Agent", "agent_phone": "(512) 555-0200",
    })
    assert low["status"] == "queued"   # below threshold -> queued
    assert high["status"] == "new"     # good photos -> not contacted


def test_listing_to_row_skipped_without_listing_agent_contact():
    row = listing_to_row({"listing_id": "3", "score": 0.2, "agent_name": "Jane Agent"})
    assert row["status"] == "skipped"
    row2 = listing_to_row({"listing_id": "4", "score": 0.2, "agent_phone": "(512) 555-0300"})
...
def test_upsert_dedup_and_status_preserved(tmp_path, monkeypatch):
    ...
    listings = [
        {"listing_id": "a", "score": 0.2, "agent_name": "A Agent",
         "agent_phone": "(210) 555-0123", "address": "1 St"},
        {"listing_id": "b", "score": 0.95, "agent_name": "B Agent",
         "agent_phone": "210-555-0199", "address": "2 St"},
    ]
...
def test_phone_normalized_on_ingest(tmp_path, monkeypatch):
    s = _store(tmp_path, monkeypatch)
    s.upsert_listings([{"listing_id": "a", "score": 0.1, "agent_name": "A Agent",
                        "agent_phone": "(210) 555-0123"}])
...
def test_get_by_status(tmp_path, monkeypatch):
    s = _store(tmp_path, monkeypatch)
    s.upsert_listings([
        {"listing_id": "a", "score": 0.1, "agent_name": "A", "agent_phone": "(210) 555-0001"},
        {"listing_id": "b", "score": 0.2, "agent_name": "B", "agent_phone": "(210) 555-0002"},
        {"listing_id": "c", "score": 0.9, "agent_name": "C", "agent_phone": "(210) 555-0003"},
    ])
